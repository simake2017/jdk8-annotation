...
public abstract class ClassLoader {
        ...
        throws ClassNotFoundException
    {
        synchronized (getClassLoadingLock(name)) {
            ...
            /**
             * 这里就是在当前的 加载器缓存中是否 加载过
             * 这里要理解一下，从当前classloader中加载，加载不到 会调用parent
             * 的该方法，但是 加载类是先调用父类加载，所以如果父类加载了，那么
             * 子类便不会在加载，所以不会有问题
             */
            Class<?> c = findLoadedClass(name);  // 在已经 加载的 类中 查找
            if (c == null) {
    ...
    */
    ...
    /*
        如果 子类 重写 loadClass 方法，不使用 双亲委派 （parent加载）
        那么，也需要使用 defineClass 方法，  下面的方法会判断  类名是否已java开头
     */
    ...
    /*
        wangyang
        获取资源，首先由父 classLoader进行加载， 加载不到 再由 子类进行加载
     */
    public Enumeration<URL> getResources(String name) throws IOException {
        ...
        tmp[1] = findResources(name); //由子类实现进行 查找
    ...
    private static URL getBootstrapResource(String name) {
        URLClassPath ucp = getBootstrapClassPath(); //获取一个 classPath
        Resource res = ucp.getResource(name); //根据classPath 获取资源
    ...
    private static synchronized void initSystemClassLoader() {
        if (!sclSet) {
            ...
            sun.misc.Launcher l = sun.misc.Launcher.getLauncher(); //加载Launcer类
            if (l != null) {
                Throwable oops = null;
