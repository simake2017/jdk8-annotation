...
public class ThreadLocal<T> {
     ...
     *
     * wangyang
     * 这里的this 就是调用 对象 这个方法属于 ThreadLocal, 所以这里的this就是ThreadLocal
     */
    ...
    static class ThreadLocalMap {
        ...
        /**
         * wangyang
         * 这里Entry是继承一个WeakReference 用于包含一个ThreadLocal
         *  这里ThreadLocal是作为一个key(WeakReference referent) ，当外部的强引用 消失的时候，这里的referent也会消失
         */
        ...
        private int expungeStaleEntry(int staleSlot) {
            ...
            /*
               这里擦去 的方式需要将 value 置为null,因为value可能本身引用一个对象
               再就是将这个entry 本身置为null才行
                擦去 删除
        ...
        private void resize() {
            ...
            for (int j = 0; j < oldLen; ++j) {
                ...
                if (e != null) {
                    ThreadLocal<?> k = e.get();
                    /**
                     * 这里是用 threadLocal 作为句柄 threadLocal会 gc 掉
                     * 但是对应的value 是强引用 不会 gc 掉，所以这里需要 设置为null
                     */
                    if (k == null) {
