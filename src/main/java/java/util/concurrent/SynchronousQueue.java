...
public class SynchronousQueue<E> extends AbstractQueue<E>
    ...
    /*
        ...
        这个类是栈的方式，后进先出， 后进的 数据  会先取出来
        阻塞式的，必须等待取出数据后，才能正常继续执行
    ...
    static final class TransferStack<E> extends Transferer<E> {
        ...
        static final class SNode {
            ...
            /**
             *  这里所谓的匹配其实就是将 请求节点 与 data节点 进行绑定
                只要通过cas绑定成功，那么就是 匹配上了
                waiter会在 await方法中 进行 阻塞 并且设置对应的值
            ...
            for (;;) {
                ...
                if (h == null || h.mode == mode) {  // empty or same-mode
                    if (timed && nanos <= 0) {      // can't wait
                        ...
                        SNode m = awaitFulfill(s, timed, nanos); //这里面 一直阻塞
                    ...
                    if (h.isCancelled())            // already cancelled
                        ...
                        for (;;) { // loop until matched or waiters disappear
                            ...
                            SNode mn = m.next; //mn是m的next，
                            if (m.tryMatch(s)) { //能够匹配成功
                                /**
                                 * 这里通过cas 将 head节点 设置为 mn（即原头结点的下一个节点），那么上面的数据节点
                                 * 和请求节点就会出队了
                                 */
                                casHead(s, mn);     // pop both s and m 将 s 和 m 同时 出队
                                return (E) ((mode == REQUEST) ? m.item : s.item);
                            } else                  // lost match
        ...
        SNode awaitFulfill(SNode s, boolean timed, long nanos) {
            ...
            for (;;) {
                 ...
                 */
                    ...
                    LockSupport.parkNanos(this, nanos);   //    阻塞一段 时间 然后 唤醒
            }
        }
