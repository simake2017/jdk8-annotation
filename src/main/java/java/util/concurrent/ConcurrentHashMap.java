...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    public ConcurrentHashMap(int initialCapacity) {
        ...
        int cap = ((initialCapacity >= (MAXIMUM_CAPACITY >>> 1)) ?
                   MAXIMUM_CAPACITY :
                   tableSizeFor(initialCapacity + (initialCapacity >>> 1) + 1)); //通过这方式，对初始容量进行 放大 ，并不会按照指定容量 初始化
        this.sizeCtl = cap;
    }
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)
                ...
                if (casTabAt(tab, i, null,
                             new Node<K,V>(hash, key, value, null))) // 设置 新的节点 ，这个位置没有，在这个位置添加一个Node
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {    // 设置   初始容量
                try {
    ...
    /*
        在这里 将 数组 的这个节点后面的链表 转为 红黑树

     */
    private final void treeifyBin(Node<K,V>[] tab, int index) {
        Node<K,V> b; int n, sc;
        if (tab != null) {
            if ((n = tab.length) < MIN_TREEIFY_CAPACITY) // 如果 数组的长度 < 64，将不会 是
                tryPresize(n << 1);
