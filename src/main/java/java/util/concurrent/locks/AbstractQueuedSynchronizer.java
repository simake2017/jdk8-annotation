...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
         ...
         * 
         */
        volatile int waitStatus; // 0 为 初始化 状态 ，初始的时候 是 0
    ...
    /*
        用于  标识 同步 状态
        每个节点 都是一个 Node  对应 一个线程
        当前节点  state 作为  互斥信号量 ，获取这个 标识 节点 出队
    ...
    private Node enq(final Node node) {
        for (;;) {
            ...
            if (t == null) { // Must initialize
                /*
                    这里要注意，在 t == null 的时候，要添加一个空节点 为 头结点
    ...
    /*
        ...
        这里是把当前线程 作为 节点 队列当中 区别于 线程池中的Worker
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws == Node.SIGNAL)
             ...
             */
            return true;
        if (ws > 0) { //大于0的 都是取消的 节点 ，通过不断往前找 取消掉
            /*
            ...
            /**
             * 这里会将 0 的状态值为 singal 状态
             *
             */
            compareAndSetWaitStatus(pred, ws, Node.SIGNAL); //对于小于0 的 设置为 Singal状态
        }
        return false;
    }
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                final Node p = node.predecessor(); // 获取 前驱 节点
                if (p == head && tryAcquire(arg)) { //如果 前驱  节点 为 head节点，则 尝试 获取锁
                    setHead(node);
                    p.next = null; // help GC
                    failed = false;
                    return interrupted;
                ...
                /*
                    在设置 完成 之后 ，将线程 LockSupport 睡眠
                 */
                /**
                 * 判断如果 当前节点的 前置节点为singal状态，则当前节点睡眠
                 */
    ...
    final boolean isOnSyncQueue(Node node) {
        if (node.waitStatus == Node.CONDITION || node.prev == null)
            return false;
        if (node.next != null) // If has successor, it must be on queue 如果有后继
            return true;
        /*
    ...
    final boolean transferForSignal(Node node) {
         ...
         * 
         */
        if (!compareAndSetWaitStatus(node, Node.CONDITION, 0))  //将 Node的 节点 的 waitStatus设置为 0 初始化状态
            return false;
         ...
         * 
         */
        Node p = enq(node);  //将该节点 加入到 阻塞队列
        int ws = p.waitStatus;
        if (ws > 0 || !compareAndSetWaitStatus(p, ws, Node.SIGNAL)) // 设置为 Signal状态
        /**
         * wangyang 状态设置为 singal 之后，解除等待状态
         */
            LockSupport.unpark(node.thread); //然后 unpark
        return true;
    }
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private Node addConditionWaiter() {
            ...
            /**
             * wangyang 这里会将 当前节点创建为一个node 并且加入到  等待队列中
             */
        ...
        private void doSignal(Node first) {
            do {
                if ( (firstWaiter = first.nextWaiter) == null)
                    lastWaiter = null;
                first.nextWaiter = null;    //     将该节点 从 条件 队列 剥离
        ...
        public final void await() throws InterruptedException {
            if (Thread.interrupted())
                throw new InterruptedException();
            Node node = addConditionWaiter();
            int savedState = fullyRelease(node); //wangyang @@@ await的时候 会将状态进行一个 释放 也就是state 状态
            int interruptMode = 0;
            while (!isOnSyncQueue(node)) { // 判定是否在同步队列上
                LockSupport.park(this);
