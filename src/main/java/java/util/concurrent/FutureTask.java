...
public class FutureTask<V> implements RunnableFuture<V> {
    ...
    private static final int NORMAL       = 2;  // 设置结果之后 就是 normal
    ...
    public void run() {
        ...
        try {
            ...
            if (c != null && state == NEW) {
                ...
                boolean ran;
                try {
                    result = c.call(); // 等  callable 执行完成 之后  ， 设置 返回 结果
                    ran = true;
                } catch (Throwable ex) {
                    result = null;
                    ran = false;
    ...
    /*
        通过 简单 链表 对所有的  futureTak 进行 进行 一个 连接
        ...
        for (WaitNode q; (q = waiters) != null;) {
            if (UNSAFE.compareAndSwapObject(this, waitersOffset, q, null)) {
                for (;;) {
                    ...
                    if (t != null) {
                        q.thread = null;
                        LockSupport.unpark(t); //wangyang 这里会把 正在等待的 线程进行唤醒
                    }
    ...
    private int awaitDone(boolean timed, long nanos)
        ...
        final long deadline = timed ? System.nanoTime() + nanos : 0L; // deadline用于表示截止期限
        WaitNode q = null;
