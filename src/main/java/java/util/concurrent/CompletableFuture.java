...
public class CompletableFuture<T> implements Future<T>, CompletionStage<T> {
    ...
    /**
     * 当 进行一次 push 的时候 会进行一个 stack 的设置
     * @param c
     * @return
     */
    ...
    final void postComplete() {
        ...
        /*
            这里是 当完成 complete 方法后进行的后处理 操作
            假设是 单线程执行的情况 首先 CompletableFuture complete之后
            stack 会以栈的形式存储一系列的 后续执行步骤 ，然后下面的循环就是
            一个一个的取出来进行 执行（fire方法），并且将下阶段设置为当前
            stack
         */
    ...
    /**
     * 这里的 参数 a 是 来源 future ，当来源future resut 不等于null
     * 的情况下 返回ture 表示可以进行下阶段消费
     * 如果 result 是 null  那么返回 false  不会进行下阶段消费
     *
     * wangyang impl
     *
     */
    ...
    /**
     *
     * 这里最终返回的 是 Future future 是一个异步结果， Stage是一个中间过程，包含src dep 以及 function 
     */
    ...
    @SuppressWarnings("serial")
    /**
     * 这里会传进来一个 Supplier接口 类 ，起到一个 获取 value的作用
     */
    ...
    @SuppressWarnings("serial")
    /**
     * 这里本身是一个 继承 Runnable的对象
     * 所以这里使用 构造方法 传进来的 Runnable只是起到一个 执行的作用
     */
