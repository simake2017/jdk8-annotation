...
//     }
    ...
    public void put(E e) throws InterruptedException {
        ...
        /*
            使用  ReentrantLock 响应中断

         */
        putLock.lockInterruptibly(); //如果线程 中断 过了 则 抛出异常
        try {
            /*
            ...
            while (count.get() == capacity) {
                notFull.await(); // 该线程节点 加入到 等待队列 中
            }
            enqueue(node); //加入到 队列 中
            c = count.getAndIncrement();
            if (c + 1 < capacity)
                notFull.signal(); //执行一下 singal
        } finally {
            putLock.unlock();
        }
        if (c == 0)
    ...
    public boolean offer(E e) {
        ...
        /*
            这里已经声明了，在offer 的 时候，如果  存在 对象 达到
            capacity ，那么直接返回 false，不会进行扩容
         */
