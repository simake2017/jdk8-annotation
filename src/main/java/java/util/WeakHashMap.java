...
public class WeakHashMap<K,V>
    ...
    /*
        ...
        要理解 reference的清除原理，下面的 Entry是继承自  WeakReference的，
         所以 在poll的时候 最终的对象 引用其实是  Entry， 派生自  WeakReference的

        传进WeakReference的 属性 referent   设置为null,但是WeakReference中
        任然存储value，与next，所以可以取出来

        这里擦除的原理就是 让 Entry.value = null
        然后让Entry在 链表里失去位置，
        至于int类型的hash，是 基础类型 ，可以忽略
