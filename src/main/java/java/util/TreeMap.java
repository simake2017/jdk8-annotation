    ...
    implements NavigableMap<K,V>, Cloneable, java.io.Serializable
{
     ...
     * @serial
     */
    private final Comparator<? super K> comparator; //设置一个节点比较器

    private transient Entry<K,V> root; //表示 根节点
    ...
    private void fixAfterInsertion(Entry<K,V> x) {
        ...
        while (x != null && x != root && x.parent.color == RED) {
            ...
            if (parentOf(x) == leftOf(parentOf(parentOf(x)))) {
                Entry<K,V> y = rightOf(parentOf(parentOf(x))); // 父节点 的 兄弟节点 （插入节点的叔伯节点 ）
                if (colorOf(y) == RED) {
                    setColor(parentOf(x), BLACK); //设置父节点为 为 黑色
                    setColor(y, BLACK); //设置 叔伯节点 为 黑色
                    setColor(parentOf(parentOf(x)), RED); //设置 爷爷 节点 为 红色，
                    x = parentOf(parentOf(x)); //然后让 x = 爷爷 节点，重新 进行 递归 循环
                } else { //如果叔伯节点是 黑色
                    if (x == rightOf(parentOf(x))) { //如果 是 父节点的右孩子，需要先 左旋 ，然后下面在右旋
                        x = parentOf(x); //找到父节点
                        rotateLeft(x); // 左旋
                    }
                    /*
                        在以该节点 为基础进行 右旋 ，然后不断 递归
                     */
