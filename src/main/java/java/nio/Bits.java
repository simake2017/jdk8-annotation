...
class Bits {                            // package-private
    ...
    static {
        long a = unsafe.allocateMemory(8); //分配8个字节 大小
        try {
            ...
            /*
                ...
                判断是 大端 还是 小端
             */
            unsafe.putLong(a, 0x0102030405060708L); // --> 放一个 16位 8字节 整数
    ...
    private static volatile long maxMemory = VM.maxDirectMemory(); //--> 直接内存 管理 ，最大直接内存
    private static volatile long reservedMemory; //--> 保留内存
    ...
    static void reserveMemory(long size, int cap) {
        synchronized (Bits.class) {
            ...
            if (cap <= maxMemory - totalCapacity) {
                reservedMemory += size; // --> 这里是 最多 预留的 内存大小
                /*
                    在使用 页对齐的情况下，比如  我上面使用的地址 刚好超过一页大一点，那么下一个页对齐
                    地址就是一页 再加上 对应的容量 cap，所以是 预留 page + cap

                 */
                totalCapacity += cap; // --> 分配的总容量
                count++; //--> 多少次 分配
                return;
            }
        }
