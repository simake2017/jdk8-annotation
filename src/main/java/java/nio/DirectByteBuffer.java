    ...
    implements DirectBuffer
{
    ...
    DirectByteBuffer(int cap) {                   // package-private
        ...
        super(-1, 0, cap, cap);
        boolean pa = VM.isDirectMemoryPageAligned(); //判断是否需要 页对齐
        int ps = Bits.pageSize(); //获取一页大小
        /*
            页对齐指的是 地址从 页的倍数开始，比如 页大小是 4k
            则 内存地址 是  4k * n，下面就是 页大小 + cap容量 大小
         */
         ...
         */
        Bits.reserveMemory(size, cap); //cap是分配的容量 size是
    ...
    /**
     * 主要是 filechannel 进行映射的时候 进行使用
     * wangyang impl
     * @param cap
     * @param addr
     * @param fd
     * @param unmapper 进行 对内存 卸载
     */
