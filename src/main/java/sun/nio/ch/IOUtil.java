...
class IOUtil {
    ...
    /**
     *
     * ioutil只是将信息 写到 os底层的channel 并不能保证
     * 对方一定收到数据
     */
        ...
        throws IOException
    {
        ...
        if (dst instanceof DirectBuffer) //这里判定如果是直接内存，直接读入进去
            return readIntoNativeBuffer(fd, dst, position, nd, lock);

        // Substitute a native buffer
        /*
            这里很明显，先要分配一个 临时的直接内存
            从直接内存在复制到 堆内存当中，不能说
            给出一个 堆内 的地址 然后 像复制堆外内存那样 复制过去，这样是不行的
            需要先获取一个堆内对象（ByeeBuffer），然后使用他的api 去复制
         */
        ByteBuffer bb = Util.getTemporaryDirectBuffer(dst.remaining()); //直接内存的 ByteBuffer
        try {
            int n = readIntoNativeBuffer(fd, bb, position, nd, lock);  //读进这里面  n是 读取的字节 数量
            bb.flip();
            /*
