...
public class URLClassPath {
        ...
        try {
            return java.security.AccessController.doPrivileged(
                ...
                public Loader run() throws IOException {
                    ...
                    if (file != null && file.endsWith("/")) {  //这种事文件目录
                        if ("file".equals(url.getProtocol())) { //以 file协议访问
                            return new FileLoader(url);
                        } else {
                            return new Loader(url);
                        }
                    } else {
                        return new JarLoader(url, jarHandler, lmap); // 以 jar协议访问
                    }
                }
            });
    ...
    /*
        代表其他协议  其他url协议  获取 文件 有可能是 http 获取
